...
// @match        https://dev.misha.vori.health/*
...
function styleMods() {
    ...
    //hide close button
    // document.getElementsByClassName('.shopping-cart .MuiButton-root:nth-child(1)')[0].style.display = 'none';

    waitForIframeLoad();
}

function waitForIframeLoad() {
    const iframe = document.querySelector('iframe#MishaFrame');
    if (iframe && iframe.contentWindow.document && iframe.contentDocument.body) {
        let iframeDoc = iframe.contentDocument.body || iframe.contentWindow.document;
        unsafeWindow.console.log(`tampermonkey iframe should be loaded`, iframe);
        waitForMishaLoad(iframeDoc);
    } else {
        unsafeWindow.console.log(`tampermonkey waiting for iframe`, iframe.contentWindow.document);
        window.setTimeout(waitForIframeLoad, 500);
    }
}

function waitForMishaLoad(iframeDoc) {
    let sidebar = iframeDoc.querySelector('.shopping-cart');
    if (sidebar) {
        unsafeWindow.console.log(`tampermonkey found close button`, sidebar);
        let button = iframeDoc.querySelector('.shopping-cart .MuiButton-root:nth-child(1)');
        button.style.display = 'none';
    } else {
        unsafeWindow.console.log(`tampermonkey waiting for misha to load`, iframeDoc);
        window.setTimeout(waitForIframeLoad, 500);
    }
}
